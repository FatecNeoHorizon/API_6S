...
COLLECTION_NAME = "consumer_units_pj"
...
_VALIDATOR = {
    "$jsonSchema": {
        ...
        "properties": {
            ...
            "uc_set": {
                "bsonType": "int",
                "description": "Consumer unit set identifier. Join key with distribution_indices. Mapped from CONJ."
            },
            "municipality_code": {
                "bsonType": "int",
            ...
            "energy": {
                "bsonType": "object",
                "description": "Monthly energy in kWh. AT: peak + off_peak. MT: total.",
                "properties": {
                    "peak": {
                        "bsonType": ["array", "null"],
                        "description": "[AT only] Peak energy — 12 monthly values (kWh). Mapped from ENE_P_01 to ENE_P_12.",
                        "items": {"bsonType": ["double", "int", "null"]}
                    },
                    "off_peak": {
                        "bsonType": ["array", "null"],
                        "description": "[AT only] Off-peak energy — 12 monthly values (kWh). Mapped from ENE_F_01 to ENE_F_12.",
                        "items": {"bsonType": ["double", "int", "null"]}
                    },
                    "total": {
                        "bsonType": ["array", "null"],
                        "description": "[MT only] Total energy — 12 monthly values (kWh). Mapped from ENE_01 to ENE_12.",
                        "items": {"bsonType": ["double", "int", "null"]}
                    }
                }
            },
...
def create(db):
    ...
    col.create_index([("location",         GEOSPHERE)], name="idx_geo")

    # Uniqueness index — encrypted hash uniquely identifies each UC
    col.create_index([("encrypted_id",     ASCENDING)], name="idx_unique_encrypted_id", unique=True)
    ...
    # UC set index — join with distribution_indices via uc_set field
    col.create_index(
        [("uc_set",        ASCENDING),
         ("voltage_level", ASCENDING)],
    ...
    col.create_index([("municipality_code", ASCENDING)], name="idx_municipality")

    # CNAE index — analysis by economic sector
    col.create_index([("cnae",              ASCENDING)], name="idx_cnae")